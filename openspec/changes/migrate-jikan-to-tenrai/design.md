...
### 4. Keep conservative pacing and honor `Retry-After`
...
For HTTP 429, inspect `Retry-After` when present and delay the next retry by at least that server-requested duration, while still applying the existing bounded retry policy. If the header is absent or invalid, fall back to the existing exponential backoff with jitter. 5xx, timeout, and connection retry behavior remains unchanged.
...
- **Ignoring `Retry-After` could prolong throttling incidents.** -> Honor valid server guidance for 429 responses while retaining bounded attempts and fallback backoff.
...
3. Add `Retry-After` handling and extend deterministic retry tests.
